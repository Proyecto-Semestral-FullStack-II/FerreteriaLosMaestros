// Archivo principal de catálogo y lógica de productos.
// Aquí se define el listado de productos, se renderizan las tarjetas,
// se aplican filtros y se gestiona el carrito de compras.
...
// Referencias a elementos del DOM usados por la página de productos.
...
// 1. RENDERIZADO DE PRODUCTOS
  ...
  const rutaImagen = `assets/Productos/${prod.codigo}.jpg`;
  const placeholder = `https://via.placeholder.com/200x160?text=${prod.codigo}`;

  const cardHTML = `
  <div class="card-producto">
    <div class="card-imagen-box">
      <a href="DetalleProducto.html?codigo=${prod.codigo}">
        <img src="${rutaImagen}" alt="${prod.nombre}" data-codigo="${prod.codigo}" data-extension="jpg" onerror="cargarImagenAlternativa(this, '${placeholder}');">
      </a>
    </div>
    <div class="card-cuerpo">
      <a href="DetalleProducto.html?codigo=${prod.codigo}" class="card-titulo" title="${prod.nombre}">
        ${prod.nombre}
      </a>
...
function cargarImagenAlternativa(imagen, placeholder) {
  const extensiones = ["jpg", "webp", "png", "avif"];
  const extensionActual = imagen.dataset.extension;
  const siguienteIndice = extensiones.indexOf(extensionActual) + 1;

  if (siguienteIndice < extensiones.length) {
    const siguienteExtension = extensiones[siguienteIndice];
    imagen.dataset.extension = siguienteExtension;
    imagen.src = `assets/Productos/${imagen.dataset.codigo}.${siguienteExtension}`;
    return;
  }

  imagen.onerror = null;
  imagen.src = placeholder;
}

  // 2. Agrega un producto al carrito y guarda la selección en LocalStorage.
  ...
  // 3. Actualiza el contador visible del carrito en la barra de navegación.
  ...
  // 4. Carga las categorías disponibles y aplica los filtros de búsqueda.
  ...
  function obtenerProductosFiltrados() {
    ...
    const categoriaSeleccionada = categoriaFilter ? categoriaFilter.value : "todas";
    const disponibilidadSeleccionada = disponibilidadFilter ? disponibilidadFilter.value : "todos";
    const precioMaximo = precioMaxFilter && precioMaxFilter.value ? Number(precioMaxFilter.value) : null;

    return productos.filter(producto => {
      const coincideTexto = !textoBusqueda ||
        producto.nombre.toLowerCase().includes(textoBusqueda) ||
        producto.categoria.toLowerCase().includes(textoBusqueda) ||
        producto.subcategoria.toLowerCase().includes(textoBusqueda) ||
      ...
      const coincideDisponibilidad = disponibilidadSeleccionada === "todos" ||
        (disponibilidadSeleccionada === "disponibles" && producto.stock > 0) ||
        (disponibilidadSeleccionada === "agotados" && producto.stock === 0);

      const coincidePrecio = precioMaximo === null || producto.precioVenta <= precioMaximo;
  ...
  if (precioMaxFilter) {
    precioMaxFilter.addEventListener("change", aplicarFiltros);
  }
  ...
  if (limpiarFiltrosBtn) {
    limpiarFiltrosBtn.addEventListener("click", () => {
      ...
      disponibilidadFilter.value = "todos";
      precioMaxFilter.value = "";
      aplicarFiltros();
    });
  }

// 5. Se ejecuta al cargar la página para inicializar filtros, productos y carrito.
